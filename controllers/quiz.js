...
exports.findOne = async (req, res) => {
    ...
    try {
        const quiz = await Quiz.findByPk(id, { rejectOnEmpty: true })
        res.json({
            message: `Quizzes update successfully with id=${id}.`,
            data: quiz,
        });
    } catch (error) {
        res.status(500).json({
...
//Menampilkan atau mengambil semua data quiz erdasarkan category tertentu
exports.getByCategoryId = async (req, res) => {
    ...
    try{
        ...
        res.json({
            message: `Quizzes update successfully with categoryId=${id}.`,
            data: quizzes,
        });
    }catch(error){
        res.status(500).json({
...
exports.getByLevelId = async (req, res) => {
    ...
    const quizzes = await Quiz.findAll({
        ...
        res.json({
            message: `Quizzes update successfully with levelId-${id}.`,
            data: quizzes,
        });
    }
